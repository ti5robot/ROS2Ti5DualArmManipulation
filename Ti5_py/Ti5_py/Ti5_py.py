...
class Ti5_py:
    ...
    def init_can(self):
        ...
        vci_initconfig = VCI_INTI_CONFIG(0x80000008,0xFFFFFFFF,0,1,0x00,0x14,0)
    ...
    def udp_read(self,sock):
        while True:
            ...
            print("received message from {client_ip} : {message}".format(client_ip = client_ip,client_port=client_port,message=message))
    ...
    def send_simple_can_command(self,num_of_actuator,can_id_list,command):
        ...
        for i in range(0,num_of_actuator):
            send.ID = can_id_list[i]
            send.Data[0] = command[i]
            ...
            if canDLL.VCI_Transmit(VCI_USBCAN2, 0, 0, byref(send), 1) == 1:
                ...
                while reclen <= 0 and cnt > 0:
                    ...
                    cnt = cnt-1
                if cnt == 0:
                    print("aaaaaa! ID %d failed after try 5 times!",send.ID)
                else:
                    for j in range(reclen):
                        data_array = [rec.STRUCT_ARRAY[j].Data[4], rec.STRUCT_ARRAY[j].Data[3], rec.STRUCT_ARRAY.Data[2],rec.STRUCT_ARRAY.Data[1]]
    ...
    def send_can_command(self,num_of_actuator,can_id_list,parameter_list):
        global canDLL
        global VCI_USBCAN2
        global STATUS_OK
        send = VCI_CAN_OBJ()
    ...
    def callback(self,msg):
        ...
        ori_position[1]=msg.tarjectory[0].joint_trajectory.points[0].positions[1]*57.3
        ori_position[2]=msg.tarjectory[0].joint_trajectory.points[0].positions[2]*57.3
        ori_position[3]=msg.trajectory[0].joint_trajectory.points[0].positions[3]*57.3
        ori_position[4]=msg.tarjectory[0].joint_trajectory.points[0].positions[4]*57.3
        ...
        next_position[5]=msg.trajectory[0].joint_trajectroy.points[n-1].positions[5]*57.3

        now_position[0]=msg.trajectory[0].joint_trajectroy.points[n-1].positions[0]*57.3*101*65536/360;
    ...
    def move_up(self):
        ...
        time.sleep(50/1e6)
        self.send_can_command(6,canidlist,cmd_v_n,para_v_n)
        time.sleep(50/1e6)

        para_pos=[0,0,0,0,0,0]
        self.send_can_command(6,canidlist,cmd_pos,para_pos)
